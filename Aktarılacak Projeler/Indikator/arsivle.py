...
`=== SERI ad satir sha ===` … `=== SON ad ===` arasındaki base64'ten kurulur,
gzip açılır, sha256 başlıktakiyle sınanır (tutmazsa ENGEL, dosya yazılmaz),
`veri/<ad>.csv.gz` yazılır ve `veri/kunye.json` güncellenir. Ağa çıkmaz."""
...
def main(kayit: Path, kosu: str, commit: str) -> int:
    ...
    while i < n:
        m = re.match(r"=== SERI (\S+) (\d+) ([0-9a-f]{64}) ===$", satirlar[i])
        if not m:
            i += 1
            continue
        ad, bar, sha = m.group(1), int(m.group(2)), m.group(3)
        j, parca = i + 1, []
        ...
        if j >= n:
            ...
            return 1
        try:
            ham = gzip.decompress(base64.b64decode("".join(parca)))
        except Exception as e:  # noqa: BLE001
            print(f"ENGEL · {ad}: açılamadı ({type(e).__name__})")
            return 1
