#!/usr/bin/env python3
"""Plotly HTML çıktılarına site ev stilini uygular.

Python projelerinin ürettiği ham Plotly HTML'leri iframe içinde dağınık
görünüyor (başlık/lejant çakışması, sabit boyut, uyumsuz renkler). Bu araç
HTML'in sonuna bir "restyle" script'i enjekte eder: sayfa yüklendikten sonra
Plotly.relayout ile ev stili uygulanır — kaynak dosyanın veri/formül kısmına
dokunulmaz, işlem idempotenttir (ikinci çalıştırma değişiklik yapmaz).

Kullanım:
  python3 site/tools/plotly_stil.py site/public/projeler/**/*.html
  python3 site/tools/plotly_stil.py --hepsi          # public/projeler altındaki tüm HTML'ler
"""

import sys
from pathlib import Path

ISARET = "<!-- tto-ev-stili -->"

EV_STILI = """
<!-- tto-ev-stili -->
<style>
  html, body { margin: 0; padding: 0; height: 100%; background: #ffffff; }
  .plotly-graph-div { width: 100% !important; height: 100vh !important; }
</style>
<script>
(function () {
  function uygula() {
    var gd = document.querySelector('.plotly-graph-div');
    if (!gd || !window.Plotly || !gd.layout) { setTimeout(uygula, 120); return; }
    var mevcut = gd.layout.title && gd.layout.title.text ? gd.layout.title.text : null;
    Plotly.relayout(gd, {
      autosize: true,
      'font.family': "-apple-system, 'Segoe UI', Helvetica, Arial, sans-serif",
      'font.size': 12.5,
      'font.color': '#211b12',
      paper_bgcolor: '#ffffff',
      plot_bgcolor: '#ffffff',
      'title.x': 0.01,
      'title.xanchor': 'left',
      'title.font.size': 15,
      'title.font.color': '#211b12',
      // lejant her zaman grafiğin ALTINDA — başlıkla çakışma imkânsız
      'legend.orientation': 'h',
      'legend.yanchor': 'top',
      'legend.y': -0.07,
      'legend.xanchor': 'left',
      'legend.x': 0,
      'legend.font.size': 11,
      'legend.bgcolor': 'rgba(255,255,255,0)',
      'margin.t': mevcut ? 92 : 36,
      'margin.r': 64,
      'margin.b': 88,
      'margin.l': 64,
      hovermode: 'x unified'
    });
    window.addEventListener('resize', function () { Plotly.Plots.resize(gd); });
    Plotly.Plots.resize(gd);
  }
  if (document.readyState === 'complete') uygula();
  else window.addEventListener('load', uygula);
})();
</script>
"""


def isle(yol: Path) -> str:
    metin = yol.read_text(encoding="utf-8", errors="ignore")
    if ISARET in metin:
        return "atlandı (zaten stilli)"
    if "plotly" not in metin.lower():
        return "atlandı (plotly değil)"
    if "</body>" in metin:
        metin = metin.replace("</body>", EV_STILI + "\n</body>", 1)
    else:
        metin += EV_STILI
    yol.write_text(metin, encoding="utf-8")
    return "stillendi"


def main():
    argv = sys.argv[1:]
    kok = Path(__file__).resolve().parents[1] / "public" / "projeler"
    dosyalar = (
        sorted(kok.rglob("*.html")) if (not argv or argv[0] == "--hepsi")
        else [Path(a) for a in argv]
    )
    for d in dosyalar:
        print(f"{d.name:36s} {isle(d)}")


if __name__ == "__main__":
    main()
